...
/** @addtogroup Template_Project
  * @{
  */
extern Tick_st TickSt;

extern __IO uint16_t SpeakCnt;

extern  WorkData_st WorkData;

extern uint32_t GetTick_1ms(void);
extern uint32_t GetTick_10ms(void);
extern uint32_t GetTick_250ms(void);
extern void SetHooktick_250ms(uint32_t * hookTick,uint16_t cnt);
extern void SetHooktick_1s(uint32_t * hookTick,int16_t cnt);
/* Private typedef -----------------------------------------------------------*/
/* Private define ------------------------------------------------------------*/
/* Private macro -------------------------------------------------------------*/
/* Private variables ---------------------------------------------------------*/
...
INTERRUPT_HANDLER(EXTI_PORTD_IRQHandler, 6)
{
  ...
  */
    ...
    //SetHooktick_1s(&WorkData.sleepTick, SLEEP_TIME);
}
...
INTERRUPT_HANDLER(TIM1_UPD_OVF_TRG_BRK_IRQHandler, 11)
{
  ...
  */
   ...
   /*
    ...
    TickSt.tick_1ms++;
    if(TickSt.tick_1ms % 1000 == 0){
        TickSt.tick_1s++;
    }

 Cleat Interrupt Pending bit
    TickSt.tick_1ms++;
    if(TickSt.tick_1ms % 10 == 0)
    {
        TickSt.tick_10ms++;
        if(TickSt.tick_10ms % 100 == 0){
         // TickSt.tick_500ms++;
