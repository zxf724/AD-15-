...
#define STR_NEQUAL(des, token)  (strcmp((char*)des, (char*)token, strlen((char*)token)) == 0)
...
/*位操作*/
#define BIT_SET(reg, bit)       (reg |= (uint32_t)(1 << bit))
#define BIT_CLEAR(reg, bit)     (reg &= ~(uint32_t)(1 << bit))
#define BIT_READ(reg, bit)      ((reg & (uint32_t)(1 << bit)) >> (uint32_t)bit)
